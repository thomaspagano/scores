...
def kge(
    ...
    *,
    ...
    Examples:
        >>> kge_s = kge(
        ...     forecasts, obs, preserve_dims="lat"
        ... )  # if data is of dimension {lat,time}, kge value is computed across the time dimension
        >>> kge_s = kge(
        ...     forecasts, obs, reduce_dims="time"
        ... )  # if data is of dimension {lat,time}, reduce_dims="time" is same as preserve_dims='lat'
        >>> kge_s = kge(
        ...     forecasts, obs, include_components=True
        ... )  # kge_s is dataset of all three components and kge value itself
        >>> kge_s_weighted = kge(
        ...     forecasts, obs, scaling_factors=(0.5, 1.0, 2.0)
