...
class Pit:
    ...
    on the closed interval :math:`[G(y-), G(y)]`. The weighted mean is itself a CDF, here denoted :math:`F`
    and called the PIT distribution for the set of forecast cases.
    Left-hand and right-hand limits of :math:`F` are accessible via the attributes ``.left`` and ``.right``.
    ...
    methods on the ``Pit`` class.

    Forecasts arguments to the ``Pit`` class can be given in two forms:

    - An ensemble for forecasts, indexed by an ensemble member dimension.
        In this case, the ensemble is interpreted as the empirical cumulative distribution
        function (CDF) generated by the ensemble members.
    - Values of the predictive CDF, indexed by a cdf threshold dimension.
        The values :math:`G(x)` of the predictive CDF :math:`G` evaluated at thresholds :math:`x` must be supplied.
        Left-hand limits :math:`G(x-)` of the predictive CDF can also be specified, allowing for
        CDFs with discontinuities. Values of the CDF between threshold coordinates :math:`x`
        are determined via linear interpolation, whilst values of the CDF at points outside
    ...
    Args:
        fcst: an xarray object of forecasts, either in giving values of the ensemble members or of the predictive CDF.
        ...
        preserve_dims: Optionally specify which dimensions to preserve when calculating the
            ...
            all dimensions to be preserved, apart from ``special_fcst_dim``.
    ...
    Attributes:
        left: xarray object giving the left-hand limits :math:`F(x-)` of the CDF :math:`F` of the
    ...
    Raises:
        ...
        ValueError: when ``fcst_left`` is not ``None``,  and ``fcst_left`` and ``fcst`` do not have identical
            shape, dimensions and coordinates.
        ValueError: when ``cdf_threshold_dim`` has been supplied and any values of
            ``fcst`` that are less then 0 or greater than 1.
        ValueError: when ``cdf_threshold_dim`` has been supplied, ``fcst_left`` is not ``None`` and any
            values of ``fcst_left`` are less then 0 or greater than 1.
        ValueError: when ``cdf_threshold_dim`` has been supplied, ``fcst_left`` is not ``None`` and
            any values of ``fcst_left`` are greater than ``fcst``.
        ValueError: when ``cdf_threshold_dim`` has been supplied and ``fcst[cdf_threshold_dim]`` is not increasing.

    Warns:
        UserWarning: when ``cdf_threshold_dim`` has been supplied, and any values of ``obs`` are less than
            the minimum of ``fcst[cdf_threshold_dim]`` or greater than the maximum of
            ``fcst[cdf_threshold_dim]``.
    ...
    def __init__(
        ...
        *,  # Force keywords arguments to be keyword-only
        ...
        if sum(x is not None for x in special_dims) != 1:
            raise ValueError(
                "A string must be supplied to exactly one of `ensemble_member_dim` and `cdf_threshold_dim`"
            )

        if ensemble_member_dim is not None:
    ...
    def plotting_points_parametric(self) -> dict:
        """
        Returns the plotting points for the graph of CDF :math:`F` of the PIT distribution.
        Points on the plot are given by :math:`(x(t),y(t))` for plotting parameter :math:`t`,
        with any gaps determined by linear interpolation.
    ...
    def expected_value(self) -> XarrayLike:
        """
        Returns the expected value of the PIT distribution :math:`F`.
    ...
    def variance(self) -> XarrayLike:
        """
        Returns the variance of the PIT distribution :math:`F`.
...
class PitFcstAtObs:
    ...
    Args:
        ...
        preserve_dims: Optionally specify which dimensions to preserve when calculating the
            PIT CDF values, where the mean is taken over all forecast cases.
            All other dimensions will be reduced. As a special case, 'all' will allow
            all dimensions to be preserved. Only one of ``reduce_dims`` and ``preserve_dims``
            can be supplied. The default behaviour if neither are supplied is to reduce all dims.
        weights: Optionally provide an array for weighted averaging (e.g. by area, by latitude,
            by population, custom) of PIT CDF values across all forecast cases.
    ...
    Raises:
        ValueError: when dimensions of ``fcst``, ``obs`` or ``weights`` contain any of the following reserved names:
            "uniform_endpoint", "pit_x_value", "x_plotting_position", "y_plotting_position", "plotting_point",
            "bin_left_endpoint", "bin_right_endpoint", "bin_centre",
        ValueError: when ``fcst_at_obs_left`` is not ``None``, and ``fcst_at_obs`` and ``fcst_at_obs_left``
            do not have identical shape, dimensions and coordinates.
        ValueError: when any values of ``fcst`` are less then 0 or greater than 1.
